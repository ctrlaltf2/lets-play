...
use super::hwdevice::DeviceContext;
use crate::helpers::*;
...
impl HwFrameContext {
	...
	pub fn get_buffer(&mut self, frame: &mut ffmpeg::frame::Video) -> Result<(), ffmpeg::Error> {
		unsafe {
			result_from_ffmpeg_return(ffmpeg::sys::av_hwframe_get_buffer(
				self.buffer,
				frame.as_mut_ptr(),
				0,
			))?;
...
impl HwFrameContextBuilder {
	...
	pub fn build(mut self) -> Result<HwFrameContext, ffmpeg::Error> {
		result_from_ffmpeg_return(unsafe { ffmpeg::sys::av_hwframe_ctx_init(self.buffer) })?;
